...
plot_dir = "/BiO2/users/jjg/SCLC/I_SCLC/Paper/221011_Github_code/Fig1/"
...
SCLC_cpm_raw = read.table("/BiO2/users/jjg/SCLC/I_SCLC/1.data_preprocess/log2_CPM_n226_nonGeneFilter.txt")[,SCLC_subtype_WTS_meta$WTS_ID]
Tuft_cell_marker = c("POU2F3", "TRPM5", "SOX9", "GFI1B", "CHAT", "ASCL2", "AVIL")
...
SCLC_cpm = read.table("/BiO2/users/jjg/SCLC/I_SCLC/Paper/221011_Github_code/expr_matrix/log2_CPM_n226.txt")
SCLC_cpm = read.table("/BiO2/users/jjg/SCLC/I_SCLC/Paper/221011_Github_code/expr_matrix/log2_CPM_n226.txt")
...
ggsave(paste0(plot_dir,"8.WTS_subtype.png"), plot = p, width = 5, height = 5)
...
p = ggplot(ggdat, aes(x = "", y = prop, fill = SCLC_subtype)) +
  ...
  scale_fill_manual(values = c("#BC3C29", "#0072B5", "#E18727", "#20854E", "#7876B1")) +
  theme_void()
ggsave(paste0(plot_dir,"8.IHC_subtype.png"), plot = p, width = 5, height = 5)
...
ggsave(paste0(plot_dir, "S2A_left_Smoking_distribution.pdf"), plot = Fig_S2A_left, width = 2, height = 3)
...
ggsave(paste0(plot_dir, "S2A_right_Smoking_distribution_Prop.pdf"), plot = Fig_S2A_right, width = 2.2, height = 3.5)
...
ggsave(paste0(plot_dir, "S2B_left_Histology_site_distribution.pdf"), plot = Fig_S2B_left, width = 3.3, height = 3)
...
ggsave(paste0(plot_dir, "S2B_right_Histology_site_distribution_Prop.pdf"), plot = Fig_S2B_right, width = 3.5, height = 3)
...
ini_stage_df = SCLC_subtype_WTS_meta[,c("ini_stage2", "SCLC_subtype")]

ini_stage_df = rbind(lapply(c("LD"), function(x) {prop_df = data.frame(table(ini_stage_df$SCLC_subtype[ini_stage_df$ini_stage2==x]));
        prop_df$Prop = prop_df$Freq/sum(prop_df$Freq); prop_df$subtype2 =rep(x, nrow(prop_df));return(prop_df)}) %>% data.frame(),
lapply(c("ED"), function(x) {prop_df = data.frame(table(ini_stage_df$SCLC_subtype[ini_stage_df$ini_stage2==x]));
        prop_df$Prop = prop_df$Freq/sum(prop_df$Freq); prop_df$subtype2 =rep(x, nrow(prop_df));return(prop_df)}) %>% data.frame())
...
ggsave(paste0(plot_dir, "S2C_right_LD_ED_distribution_Prop", round(fisher_label,4),".pdf"), plot = Fig_S2C_right, width = 2.2, height = 3.5)
...
ggsave(paste0(plot_dir, "S2C_left_LD_ED_distribution.pdf"), plot = Fig_S2C_left, width = 1.8, height = 3)
...
pdf(paste0(plot_dir, "Fig2A.OS_Analysis_SCLC_subtype_LD.pdf"), width = 8, height = 6.2)
p = ggsurvplot(
  fit = OS_fit,
  size = 1.5,
...
pdf(paste0(plot_dir, "Fig2B.OS_Analysis_SCLC_subtype_ED.pdf"), width = 8, height = 6.2)
p = ggsurvplot(
  fit = OS_fit,
  size = 1.5,
  xlab = "Months since Treatment",
...
pdf(paste0(plot_dir, "Fig2C.OS_Analysis_SCLC_subtype_ED_CTX.pdf"), width = 8, height = 6.2)
p = ggsurvplot(
  fit = OS_fit,
  size = 1.5,
  xlab = "Months since Treatment",
...
pdf(paste0(plot_dir, "Fig2D.OS_Analysis_SCLC_subtype_ED_IO.pdf"), width = 8, height = 6)
p = ggsurvplot(
  fit = OS_fit,
  size = 1.5,
  xlab = "Months since Treatment",
...
pdf(paste0(plot_dir, "FigS3A.ctx_1st_pfs_SCLC_subtype_ED.pdf"), width = 8, height = 6.2)
p = ggsurvplot(
  fit = Pfs_fit,
  size = 1.5,
  xlab = "Months since Treatment",
...
pdf(paste0(plot_dir, "FigS3B.PFS_SCLC_subtype_ED_CTx_only.pdf"), width = 8, height = 6.2)
p = ggsurvplot(
  fit = Pfs_fit,
  size = 1.5,
  xlab = "Months since Treatment",
...
pdf(paste0(plot_dir, "FigS3C_.PFS_SCLC_subtype_ED_CTx_IO.pdf"), width = 8, height = 6)
p = ggsurvplot(
  fit = Pfs_fit,
  size = 1.5,
  xlab = "Months since Treatment",
...
ggsave(paste0(plot_dir, "S1C_ASCL1_NEUROD1_H_score_scatterPlot.pdf"), plot = p, width = 7, height = 7)
...
ggsave(paste0(plot_dir, "S1C_ASCL1_NEUROD1_log2cpm_scatterPlot.png"), plot = p, width = 7, height = 7)
...
ggsave(paste0(work_dir, "/Fig_S1H_NoEdgeR_Filter_Tuft_gene_wilcox(A&trans&P_vs_N).png"), plot = p + axis_theme, width = 4, height = 4)
