...
#' @export
library(maftools)
Custom_oncoplot = oncoplot = function(maf, top = 20, minMut = NULL, genes = NULL, altered = FALSE,
  ...
  if(!is.null(genes)){ #If user provides a gene list
    om = Custom_createOncoMatrix(m = maf, g = genes, add_missing = fill, cbio = cBioPortal)
    ...
    om = Custom_createOncoMatrix(m = maf, g = genes, cbio = cBioPortal)
    numMat = om$numericMatrix
    mat_origin = om$oncoMatrix
  ...
  annotation_order = parse_annotation_dat(annotationDat = maf, clinicalFeatures = clinicalFeatures[1])
  annotation_order = annotation_order[colnames(numMat) %>% unique(),] %>% table() %>% data.frame()
  annotation_order = annotation_order[rev(order(annotation_order[,2])),]
  ...
  if(drawColBar & is.null(topBarData)){
    ...
    abline(v = lapply(seq(1, nrow(annotation_order)), function(x) {return(sum(annotation_order[seq(1,x),2]))}) %>% unlist() %>% as.numeric(), col = borderCol, lwd = sepwd_groups)
    if(logColBar){
  ...
  abline(v = lapply(seq(1, nrow(annotation_order)), function(x) {return(sum(annotation_order[seq(1,x),2]))}) %>% unlist() %>% as.numeric() + 0.5, col = borderCol, lwd = sepwd_groups)
