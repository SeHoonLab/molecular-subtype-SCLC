CustomSummarizeMaf = function(maf, anno = NULL, chatty = TRUE){
  ...
  maf.tsbs = unique(anno[,Tumor_Sample_Barcode])
  nSamples = length(unique(anno$Tumor_Sample_Barcode))
  ...
  if(any(colnames(vc.cast) %in% c('Amp', 'Del'))){
    ...
    vc.cast$total = vc.cast.cnv$CNV_total

    vc.cast = merge(vc.cast, vc.cast.cnv, by = 'Tumor_Sample_Barcode', all = TRUE)[order(CNV_total, decreasing = TRUE)]
  ...
  if(any(colnames(vt.cast) %in% c('CNV'))){
    ...
    vt.cast$total = vt.cast.cnv$CNV

    vt.cast = merge(vt.cast, vt.cast.cnv, by = 'Tumor_Sample_Barcode', all = TRUE)[order(CNV, decreasing = TRUE)]
  }else{
  ...
  if(any(colnames(hs.cast) %in% c('Amp', 'Del'))){
    ...
    hs.cast$total = hs.cast.cnv$CNV_total

    hs.cast = merge(hs.cast, hs.cast.cnv, by = 'Hugo_Symbol', all = TRUE)[order(CNV_total, decreasing = TRUE)]
  }else{
  ...
  #Merge and sort
  hs.cast = merge(hs.cast, numAlteredSamples, by = 'Hugo_Symbol', all = TRUE)[order(AlteredSamples, decreasing = TRUE)]
  #Replace NAs with 0
