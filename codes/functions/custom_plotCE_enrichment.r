...
#'
custom_plotEnrichmentResults = function(enrich_res, pVal = 0.05, cols = NULL, annoFontSize = 0.8, geneFontSize = 0.8, legendFontSize = 0.8, showTitle = TRUE){
  ...
  plot.dat$Hugo_Symbol
  plot.dat = plot.dat[rev(order(plot.dat$Group1, plot.dat$g1_muts)),]
  plot.dat = plot.dat[order(plot.dat$Group1),]
  #NOTCH1_include = lapply(seq(1, length(plot.dat$P_value)), function(x) {seq(1,length(plot.dat$P_value))[order(plot.dat$P_value)==x]}) %>% unlist() %>% as.numeric()
  #NOTCH1_include = NOTCH1_include==min(NOTCH1_include[plot.dat$Hugo_Symbol=="NOTCH1"])
  #plot.dat = plot.dat[P_value < pVal | NOTCH1_include]
