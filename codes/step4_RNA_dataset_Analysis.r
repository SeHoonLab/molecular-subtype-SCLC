...
SCLC_cpm =read.table("../Figures/step1/log2_CPM_n226.txt", sep = "\t")
...
set.seed(100)
N_Atrans_DEG = readRDS("../Figures/step1/Fig_S5B_edgeR_N_Atrans_DEG.Rds")
...
ggsave(paste0(work_dir, "Fig4B_N_Atrans_DEG_pvalue_fgsea.png"), plot = p, width = 9, height=5)
...
set.seed(100)
P_Atrans_DEG = readRDS("../Figures/step1/Fig_S5A_edgeR_P_Atrans_DEG.Rds")
...
ggsave(paste0(work_dir, "Fig4A_P_A&trans_DEG_pvalue_fgsea.png"), plot = p, widt=8.4, height=5)
...
pdf(paste0(work_dir, "Fig5A_TME_annotated_inflamed_clustering_complexheatmap.pdf"), width = 14, height = 10)
...
pdf(paste0(work_dir, "Fig5C_first_line_IO_sample_Survplot_by_inflamed.pdf"), width = 8, height = 6)
        p = ggsurvplot(
                fit = pfs_fit,
                size = 1.5,
...
p = p + theme(axis.text.x = element_text(angle = 20, vjust = 0.5)) + xlab("") + ylab("") + ggtitle(paste0("Fisher p.value: ",round(fisher_pvalue$p.value, 3)))
ggsave(paste0(work_dir, "Fig5D_InflamedInfo_proportion_by_responder.png"), plot=p, width = 4, height=4)
...
p = p + theme(axis.text.x = element_text(angle = 20, vjust = 0.5)) + xlab("") + ylab("") + ggtitle(paste0("Fisher p.value: ",round(fisher_pvalue$p.value, 3)))
ggsave(paste0(work_dir, "Fig5D_TME_subtype_proportion_by_responder.png"), plot=p, width = 3.5, height=4)
...
ggsave(paste0(work_dir, "Fig5E_TME_subtype_signature_facet_boxplot.png"), plot=p, width = 12, height=6)
...
ggsave(paste0(work_dir, "Fig_S6A_182_samples_Inflamed_nonInflamed_VolcanoPlot.png"), plot = plot_Inflamed_nonInflamed_DEG, width = 5, height = 6)
...
p = ggplot(fgseaRes[grepl("NOTCH", fgseaRes$pathway) | c(!is.na(fgseaRes$pval) & abs(fgseaRes$NES)>=1 & fgseaRes$pval < 0.1),]) +
  ...
  theme_bw() + scale_fill_gradient(low = "#ff0000", high = "#0000FF") + axis_theme + scale_fill_gradientn(limits = c(0,3), colours=c("blue", "red"))
ggsave(paste0(work_dir, "Fig_S6B_182_samples_Inflamed_nonInflamed_fgsea.png"), plot = p, widt=8.4, height=5)
...
pdf(paste0(work_dir, "Fig_S6C_pfs_by_A&trans_versus_N_Survplot_in_IO.pdf"), width = 8, height = 6)
        p = ggsurvplot(
                fit = pfs_fit,
                size = 1.5,
                xlab = "Months since Treatment",
...
pdf(paste0(work_dir, "Fig_S6D_pfs_by_TME_subtype3_Survplot_in_IO.pdf"), width = 8, height = 6)
        p = ggsurvplot(
                fit = pfs_fit,
                size = 1.5,
                xlab = "Months since Treatment",
...
pdf(paste0(work_dir, "Fig_S6F_NOTCH_and_Immune_sample_distribution.pdf"), width = 9, height = 12)
